...
static int cdn_dp_set_pattern(struct cdn_dp_device *dp, u8 dp_train_pat)
{
	u32 phy_config, global_config;
	int ret;
	u8 pattern = dp_train_pat & DP_TRAINING_PATTERN_MASK;
...
static u32 cdn_dp_select_chaneq_pattern(struct cdn_dp_device *dp)
{
	 ...
	 * follow the spec.
	 */
	tps3_supported = drm_dp_tps3_supported(dp->dpcd);

	dev_info(dp->dev, "tps3_supported=%d\n", tps3_supported);

	if (dp->link.rate == DP_LINK_BW_5_4 && tps3_supported)
...
static int cdn_dp_link_training_clock_recovery(struct cdn_dp_device *dp)
{
	...
	for (;;) {
		...
		if (drm_dp_clock_recovery_ok(link_status, dp->link.num_lanes)) {
			dev_info(dp->dev, "clock recovery OK\n");
			return 0;
		}

		if (voltage_tries >= 5) {
			dev_err(dp->dev, "Same voltage tried 5 times\n");
			return -EINVAL;
		}

		if (max_vswing_tries >= 1) {
			dev_err(dp->dev, "Max Voltage Swing reached\n");
			return -EINVAL;
		}
		...
		if (cdn_dp_update_link_train(dp)) {
			dev_err(dp->dev, "failed to update link training\n");
			return -EINVAL;
		}
...
static int cdn_dp_link_training_channel_equalization(struct cdn_dp_device *dp)
{
	...
	if (ret) {
		dev_err(dp->dev, "failed to start channel equalization\n");
		return ret;
	}
	...
	for (tries = 0; tries < 5; tries++) {
		...
		if (!drm_dp_clock_recovery_ok(link_status,
					      dp->link.num_lanes)) {
			dev_err(dp->dev, "Clock recovery check failed\n");
			break;
		}

		if (drm_dp_channel_eq_ok(link_status,  dp->link.num_lanes)) {
			dev_info(dp->dev, "Channel EQ done\n");
			return 0;
		}
	...
	if (tries == 5)
		dev_err(dp->dev, "Channel equalization failed 5 times\n");
...
int cdn_dp_software_train_link(struct cdn_dp_device *dp)
{
	...
	ret = drm_dp_dpcd_read(&dp->aux, DP_DOWNSPREAD_CTRL, &downspread, 1);
	if (ret < 0) {
		DRM_DEV_ERROR(dp->dev, "Failed to get downspread %d\n", ret);
		return ret;
	}

	dev_info(dp->dev, "downspread=%0x\n", downspread);
	...
	while (true) {
		...
		dev_info(dp->dev, "rate=%02x, lanes=%d\n",
		...
		if (ret) {
			...
			dev_err(dp->dev, "training clock recovery failed: %d\n", ret);
			break;
		}
		...
		if (ret) {
			...
			dev_err(dp->dev, "training channel eq failed: %d\n", ret);
			break;
		}
